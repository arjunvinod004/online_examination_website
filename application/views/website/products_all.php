...
<div class="weekly-best-selling-area bg_light-1">
                        ...
                        <ul class="nav-h_top language p-0">
                            ...
                            <div class="row">
                                <div class="col-lg-12 text-center">
                                    <img class="logo" src="<?php if(isset($store_informations->store_logo_image)){ echo base_url()."uploads/store/".$store_informations->store_logo_image;} ?>" alt="Veg">
                                    <h4 style="color: #914747;font-size: 18px;" class="title-left text-uppercase mb-0"><?php echo $store_informations->store_name; ?> </h4>
                                </div>
                                <div class="col-lg-12">
  ...
  <p><?php echo $store_informations->store_desc; ?></p>
        ...
        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
            <li>
                <a onclick="loadProductsCategoryFilter('all')" class="dropdown-item category" href="#">
                    <i class="bi bi-eye"></i> All
                </a>
            </li>
            <?php foreach($categories as $category){ ?>
                <li>
                <a onclick="loadProductsCategoryFilter(<?php echo $category['category_id']; ?>)" class="dropdown-item category" href="#">
                    <i class="bi bi-eye"></i> <?php echo $category['category_name_'.$language]; ?>
                </a>
            </li>
            <?php } ?>
        </ul>
        ...
        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
            ...
            <?php foreach($subcategories as $category){ ?>
                <li>
                <a onclick="loadProductsSubCategoryFilter(<?php echo $category['subcategory_id']; ?>)" class="dropdown-item category" href="#">
                    <i class="bi bi-eye"></i> <?php echo $category['subcategory_name_'.$language]; ?>
                </a>
            </li>
            <?php } ?>
        </ul>
                                            ...
                                            <li class="nav-item" role="presentation">
                                                <button class="nav-link active clear" style="padding:5px;" type="button">Clear</button>
                                            </li>
                                        </ul>
...
<div class="fixed-cart-total" style="position:fixed;bottom:0px;width:100%;background: #ef4f5f;color: #fff;text-align: center;padding: 10px 12px;font-size: 15px; display: none; align-items: center; justify-content: space-between; gap: 10px;">
...
<!--<a href="<?php echo site_url() ?>cart/view"><p class="cart-total" style="color: #fff;font-size: 16px;font-size: 16px;font-weight: bold;"><span id="total-items"> PLACE ORDER <span id="cart-total"></span>  </p></a>-->
    ...
    <!-- Modal structure -->
  <div class="modal fade" id="productDetails" tabindex="-1" aria-labelledby="productCustomizeLabel" aria-hidden="true">
    ...
    <div class="modal fade" id="productCustomize" tabindex="-1" role="dialog" aria-labelledby="foodModalLabel" data-bs-backdrop="static" aria-hidden="true" data-bs-keyboard="false">
    ...
    function displayCart(cartData) {
        ...
        function sumOfDigits(number) {
            ...
            if(cartTotal != 0)
            {
                    ...
                    $('#total-items').html(' YOUR ORDER <br><span id="total-cost">Total Cost : ₹' + cartTotal.toFixed(2) + '</span>' );
            }
            else
            {
...
//alert(data)
                $('.variant').each(function () {
                    ...
                    if(quantity > 0){
                        ...
                        $.ajax({
                            ...
                            method: 'POST',
                            data: { prdParentId : productId, product_id: productId, quantity: quantity, name : 'name' , image: 'image' , addon: 0 , recipe: recipe , variant_id:variantId , price: price , variantIds : variantIds  },
                            success: function(data) {
                                        ...
                                        $('.addon').each(function () {
                                            //alert('addon');
                                            const price = parseFloat($(this).data('price'));//alert(price);
                                            const itemId = parseFloat($(this).data('id')); 
                                            const quantity1 = parseInt($(this).find('.addon-qty').val()) || 0;
                                            if(quantity1 > 0){
                                                //alert(1222);
                                                addonIds.push({
                                                    id: itemId,
                            ...
                            method: 'POST',
                            data: { prdParentId : productId, product_id: productId, quantity: quantity, name : 'name' , image: 'image' , addon: 1 , recipe: 'recipe' , variant_id:itemId , price: price , addonIds : addonIds  },
                            success: function(data) {
                ...
                //location.href = '<?= base_url('website/cart'); ?>';
