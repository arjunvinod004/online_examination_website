...
<div class="weekly-best-selling-area bg_light-1">
  ...
  <p>Lorem ipsum dolor sit amet consectetur adipisicing elit.!</p>
            ...
            <h2 class="accordion-header" id="heading<?php echo $key; ?>">
                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse<?php echo $key; ?>" aria-expanded="false" aria-controls="collapse<?php echo $key; ?>"><?php echo $category['category_name_'.$language]; ?></button>
            </h2>
            <div id="collapse<?php echo $key; ?>" class="accordion-collapse collapse" aria-labelledby<?php echo $key; ?>" data-bs-parent="#accordionExample">
                                            ...
                                            <div class="col-xl-4 col-lg-4 col-md-4 col-sm-6 col-12">
                                                    <div class="product-grid">
                                                                ...
                                                                <div class="left-column" style="width: 65%;float: left;">
                                                                    <a href=""><h4 class="title"><?php echo $product['product_name_' . $language]; ?></h4></a>
                                                                    <div class="product-description"><?php echo $product['product_desc_' . $language]; ?>.</div>
...
<img src="<?php echo site_url() ?>uploads/product/<?php echo $product['image']; ?>" data-bs-toggle="modal" data-bs-target="#productCustomize" data-prodId="<?php echo $product['store_product_id']; ?>" data-id="<?php echo $product['product_id']; ?>" alt="Product Image" class="product-image customize">
 <!-- Add button -->
                                                                    <div class="product" data-id="<?php echo $product['product_id']; ?>" data-price="<?php echo $product['price']; ?>">
<input type="hidden" name="product_id" value="<?php echo $product['product_id']; ?>">
    <button data-id="<?php echo $product['product_id']; ?>" class="add-button" data-bs-toggle="modal" data-bs-target="#productCustomize">ADD</button>
    </div>                                                             
                                                               <?php }else{ ?>
                                                                <img src="<?php echo site_url() ?>uploads/product/<?php echo $product['image']; ?>" alt="Product Image" class="product-image">
                                                                 <!-- Add button -->
                                                                    <div class="product" data-id="<?php echo $product['product_id']; ?>" data-price="<?php echo $product['price']; ?>">
<input type="hidden" name="product_id" value="<?php echo $product['product_id']; ?>">
    <button class="add-button add-to-cart" onclick="addToCart(this)">ADD</button>
    <div id="quantityControls" class="add-button" style="display: none;">
...
$(document).ready(function() {
  ...
  $('#productCustomize').on('show.bs.modal', function(event) {
    ...
    $.ajax({
      ...
      type: "POST",
      data: { product_id: productId , store_id : $('#store_id').val() , language: $('#language').val(),prod : product },
      success: function(response) {
        //alert(response);
